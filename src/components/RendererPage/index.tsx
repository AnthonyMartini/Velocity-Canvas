...
export default function RendererPage({ user, onCreditDeduction, activeProject, projectSessionKey, setActiveProject }: { user: any, onCreditDeduction?: () => void, activeProject: any, projectSessionKey: string, setActiveProject: (p: any) => void }) {
  ...
  const updateProp = useCallback((id, key, val, options: any = null) => {
    setTree(prev => {
      const next = updateNode(prev, id, (node) => {
        const nextFormulaProps = node?._formulaProps && typeof node._formulaProps === 'object'
          ? { ...node._formulaProps }
          : {}

        if (options?.formula === true) {
          nextFormulaProps[key] = true
        } else if (options?.formula === false) {
          delete nextFormulaProps[key]
        }

        return {
          [key]: val,
          _formulaProps: Object.keys(nextFormulaProps).length ? nextFormulaProps : null,
        }
      })
  ...
  const handleCanvasPanStart = useCallback((e) => {
    ...
    const shouldPan = e.button === 2 || e.button === 1 || (e.button === 0 && spacePanRef.current)
      ...
      <div className="flex-1 relative overflow-hidden" style={{ backgroundColor: themeVars.colors.canvasWorkspace }}>
          ...
          <div
            ...
            onMouseDown={(e) => {
              ...
              if (handleCanvasPanStart(e)) return
                    ...
                    {schema.properties
                      ...
                      .map(prop => (
                        <PropField 
                          ...
                          onChange={(val, options) => updateProp(selectedNode.id, prop.key, val, options)}
