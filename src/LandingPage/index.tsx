...
export default function LandingPage({ onStart }) {
  const handleGoogleSignIn = async () => {
    try {
      ...
      console.log('Attempting Firestore read for user:', user.uid);
      const existing = await getDoc(userRef);
      console.log('Firestore read success, existing:', existing.exists());
      ...
      console.error('--- Firebase Error Diagnostic ---');
      ...
      // Try to proceed anyway if auth settled
      if (auth.currentUser) {
        onStart('generator', auth.currentUser);
      }
    }
  };
  return (
